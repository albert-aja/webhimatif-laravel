...
class SocialMediaController extends AdminController
{
	public function __construct(){
		parent::__construct();
		$this->data['page'] = ['page' => __('admin/crud.variable.social_media')];
	}

	private function prepare_iconArray($data, $current = ''){
        $icon_arr = Icon::icon_array();

		foreach($data->pluck('icon') as $icon){
    ...
    public function index(){
		...
		if(request()->ajax()){
            return Datatables::of(Social_Media::query())
					...
					->addColumn('action', function($item){
						return '<div class="dropdown d-inline">
									...
									<div class="dropdown-menu">
										<a type="button" class="dropdown-item has-icon editSocial" data-id="' .$item->id. '">
											<i class="fas fa-pen"></i> ' .__('admin/crud.btn.edit'). '
										</a>
										<a type="button" class="dropdown-item has-icon deleteSocial" data-id="' .$item->id. '" data-social="' .$item->social. '">
	...
	public function create(){
        ...
        $this->data['existed']  = Social_Media::all();
        $this->data['icon_arr'] = self::prepare_iconArray($this->data['existed']);

		return view('v_admin.config.social.modal_add', $this->data);
	}
    ...
    public function store(Request $request){
		...
		if(!empty($val->errors()->messages())){
			...
			Social_Media::create($request->all());
	...
	public function edit(Request $request){
        ...
        $this->data['existed'] = Social_Media::all()->except($request->id);
        $this->data['social'] = Social_Media::where('id', $request->id)->first();
        $this->data['icon_arr'] = self::prepare_iconArray($this->data['existed'], $this->data['social']);

		return view('v_admin.config.social.modal_edit', $this->data);
	}
	...
	public function update(Request $request){
		...
		if(!empty($val->errors()->messages())){
			...
			$item = Social_Media::findOrFail($request->id);
    ...
    public function destroy(Request $request){
		Social_Media::findOrFail($request->id)->delete();
    }
    ...
    private function validator(array $data, string $id = ''){
        return Validator::make($data, [
			'social'	=> 'required|unique:social__media' .(($id) ? ',social,'.$id : ''),
